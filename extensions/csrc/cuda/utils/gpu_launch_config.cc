...
GPULaunchConfig GetGPULaunchConfig1D(const NVGPUDevInfo& dev_info,
  ...
  int64_t active_threads = (numel + vec_size - 1) / numel;
  ...
  GPULaunchConfig gpu_launch_config;
  dim3 block(expected_threads_per_block, 1, 1);
  dim3 grid(expect_block_per_grid, 1, 1);
  gpu_launch_config.set_block(block);
  gpu_launch_config.set_grid(grid);
  return gpu_launch_config;
}
